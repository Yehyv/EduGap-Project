...
const RatingReview = lazy(() => import("@/shared/components/ui/RatingReview"));
...
const quickLinks = [
  { icon: StarIcon, label: "الدروس الهامة" },
  { icon: WriteIcon, label: "ملاحظاتي" },
  { icon: AttachementIcon, label: "مرفقات الدورة" },
];
  ...
  return (
    <div className="w-full lg:w-[30%] min-lg:min-h-full min-lg:h-[400px]">
          ...
          <Suspense
            fallback={
              <p className="text-center">
                <ButtonLoader borderColor="border-black" />
              </p>
            }
          >
            {faqs.map((faq, i) => (
      ...
      {quickLinks.map(({ icon: Icon, label }, i) => (
        <h5
          key={i}
          className="cursor-pointer mt-2 flex gap-2 shadow-custom overflow-auto top-16 rounded-xl px-5 py-2 font-semibold"
        >
      ...
      <div className="shadow-custom overflow-auto top-16 rounded-xl py-5">
        ...
        <section className="px-6 my-2 text-center">
          ...
          <Link
            className="border inline-block mt-4 border-secondary px-10 rounded-lg py-0.5 text-secondary"
            to="/expert/1"
          >
            المزيد عن الخبير
          </Link>
      ...
      <div className="shadow-custom overflow-auto top-16 rounded-xl py-5">
        <h5 className="flex gap-2 text-lg font-semibold mb-4 border-b pb-3 border-[#D0CDCD] px-5">
          <RateIcon />
          <span>قيم الدورة</span>
        </h5>
        <section className="px-6 my-2 text-center">
          <p className="mb-2">
            برجاء تقييم الدورة من حيث المحتوي والدروس والجودة والفائدة
          </p>
          <Suspense fallback={<p className="text-center">Loading rating...</p>}>
            <RatingReview />
          </Suspense>
        </section>
      </div>
