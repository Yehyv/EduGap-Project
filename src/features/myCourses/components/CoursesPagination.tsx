...
import { getMyCurrentCourses } from "../services/myCoursesApis";

const CurrentCoursesPagination = ({ queryKey, queryFunc }) => {
  ...
  const { data, isLoading, error } = useQuery<CoursesResponse>({
    ...
    queryFn: () => queryFunc(page.toString(), RESULTS_PER_PAGE),
  });
...
export default CurrentCoursesPagination;
