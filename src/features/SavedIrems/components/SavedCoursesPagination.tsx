...
import { getAllSavedContents } from "@/features/GuestHome/services/GuestHomeApi";
...
const SavedCoursesPagination = () => {
  const [searchParams, setSearchParams] = useSearchParams();
  const page = Number(searchParams.get("page")) || 1;
  // ToDo : get courses not content
  const { data, isLoading, error } = useQuery<CoursesResponse>({
    queryKey: ["getSavedCoursesList", page],
    queryFn: () => getAllSavedContents(page.toString(), RESULTS_PER_PAGE),
  });
...
export default SavedCoursesPagination;
