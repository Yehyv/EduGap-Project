...
import {
  ...
  getLatestCoursesListForGuest,
} from "@/features/GuestHome/services/GuestHomeApi";
...
const LatestCoursesPageForGuest = () => {
  const { t } = useLanguage();
  const [searchParams, setSearchParams] = useSearchParams();
  const page = Number(searchParams.get("page")) || 1;
  const { data, isLoading, error } = useQuery<CoursesResponse>({
    queryKey: ["getLatestCoursesListForGuest", page],
    queryFn: () =>
      getLatestCoursesListForGuest(page.toString(), RESULTS_PER_PAGE),
  });
...
export default LatestCoursesPageForGuest;
